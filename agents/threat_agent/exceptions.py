...
══════════════════════════════════════════════════════════════════════════════
TRC Engine — Phase 1: Threat Agent  |  Typed Exception Hierarchy
──────────────────────────────────────────────────────────────────────────────
All typed exceptions for the Threat Agent live here.

Design rationale
────────────────
• Clean Architecture requires that every inter-module error is a typed,
  named exception — not a bare ValueError or RuntimeError.  Callers can
  catch by type, not by message string.
• Centralising exceptions in one file prevents circular imports (e.g.
  retrieval.py and attack_chain.py both needing the same exception class).
• Every exception carries a human-readable message AND structured fields
  where they add actionable context for the retry loop or audit log.

Consumer map
────────────
  KBStoreUnreachableError  → retrieval.py, attack_chain.py
  EmptyKBMatchError        → retrieval.py
  MalformedKBEntryError    → retrieval.py, kb/loaders/*
  KBEntryNotFoundError     → retrieval.py (get_kb_entry)
  EmptyAttackPathError     → attack_chain.py, generator.py
  AttackChainError         → attack_chain.py
  MalformedAssetInputError → retrieval.py (plan validation)
"""

from __future__ import annotations

# ══════════════════════════════════════════════════════════════════════════════
# § 1  —  KB / Retrieval exceptions
# ══════════════════════════════════════════════════════════════════════════════
